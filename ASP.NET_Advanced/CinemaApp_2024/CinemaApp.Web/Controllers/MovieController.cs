...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        ...
        public async Task<IActionResult> Create(AddMovieInputModel inputModel)
        {
            ...
            {
                this.ModelState.AddModelError(nameof(inputModel.ReleaseDate), String.Format("The Release Date must be in the fallowing format: {0}", ReleaseDateFormat));
            }

            if (!this.ModelState.IsValid)
            {
                //  Renders the same form with user entered values + model errors
