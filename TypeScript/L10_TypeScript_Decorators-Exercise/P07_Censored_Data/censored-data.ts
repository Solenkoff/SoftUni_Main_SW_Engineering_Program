...
// let userCensorService = new MockCensorService<User>(['age'])                       //  Second Input
...
function getLatest(seconds: number){
    return function(target: any, methodName: string, descriptor: PropertyDescriptor){
        ...
        descriptor.value = function(){
            ...
            const recentItems = allItems.filter((itme: any) => currentTime.getTime() - itme.createdOn.getTime() <= (seconds * 1000));
...
function logMessage(target: any, methodName: string, descriptor: PropertyDescriptor){
    const originalMethod = descriptor.value;
