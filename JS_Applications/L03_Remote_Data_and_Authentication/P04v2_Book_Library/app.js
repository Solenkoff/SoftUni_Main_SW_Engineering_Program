...
async function onLoad() {
    ...
    Object.entries(data).forEach(([id, book]) => {
        const trElement = createTableRow(id, book.title, book.author);
...
function onEdit(event) {
    ...
    const bookId = trElement.dataset.id;
...
function createTableRow(id, author, title) {
    ...
    const btnTdEl = createEl('td');
    const editBtn = createEl('button', 'Edit');
    const deleteBtn = createEl('button', 'Delete');
    btnTdEl.appendChild(editBtn);
    btnTdEl.appendChild(deleteBtn);

    editBtn.addEventListener('click', onEdit);
    deleteBtn.addEventListener('click', onDelete);

    trElement.appendChild(btnTdEl);
